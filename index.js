...
const addNewGoal = () => {
    ...
    const existingGoal = Array.from(goalList.querySelectorAll('li')).map(li => li.textContent);   //check existing goals from list

    if (existingGoal.includes(goalInput)) {
        alert('Goal already exists')     //Alert if duplicate
        return;
    }
